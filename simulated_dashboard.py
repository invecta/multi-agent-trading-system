#!/usr/bin/env python3
"""
Working Backtesting Dashboard with Simulated Data
Guaranteed to work - uses realistic simulated data
"""
import dash
...
def generate_simulated_data(symbol, start_date, end_date):
    ...
    print(f"Generating simulated data for {symbol} from {start_date} to {end_date}")
    ...
    # Filter out weekends (simulate trading days only)
...
def calculate_technical_indicators(df):
    ...
    if df is None or len(df) < 50:
        print(f"Not enough data for technical indicators: {len(df) if df is not None else 0} rows")
        return df
...
def generate_trading_signals(df):
    ...
    if df is None or len(df) < 50:
        print(f"Not enough data for signals: {len(df) if df is not None else 0} rows")
        return df
...
def run_backtest(df, initial_capital=100000):
    ...
    for i in range(1, len(df)):
        ...
        if signal == 1 and position == 0:  # Buy signal
            ...
            trades.append({
                'Date': df.index[i].date().strftime('%Y-%m-%d'),
                'Type': 'BUY',
...
app.layout = html.Div([
    html.Div([
        ...
        html.P("Using Simulated Data - Guaranteed to Work!", style={'textAlign': 'center', 'color': '#27ae60', 'fontSize': '16px', 'fontWeight': 'bold'})
    ]),
...
def run_backtest_callback(n_clicks, symbol, start_date, end_date, initial_capital):
    ...
    try:
        ...
        if df is None:
            error_msg = f"Could not generate data for {symbol}"
        ...
        portfolio_fig.update_layout(
            title=f"Portfolio Performance - {symbol} (Simulated)",
        ...
        drawdown_fig.update_layout(
            title=f"Drawdown Analysis - {symbol} (Simulated)",
            xaxis_title="Trading Days",
        ...
        risk_fig = px.bar(
            ...
            title=f"Risk Metrics - {symbol} (Simulated)",
            template="plotly_white",
...
def update_comparison(n_clicks, symbols):
    ...
    try:
        ...
        if comparison_data:
            ...
            fig.update_layout(
                title="Multi-Symbol Performance Comparison (Simulated)",
                xaxis_title="Symbol",
...
if __name__ == "__main__":
    ...
    print("Using Simulated Data - Guaranteed to Work!")
